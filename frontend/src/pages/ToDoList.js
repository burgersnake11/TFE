...
const TodoList = () => {
  ...
  useEffect(() => {
    let id_principales = []
    let id_secondaires = []
    axios.get("http://54.37.9.74:3001/todo", { params: { "commande_id": commandNumber } }).then((res) => {
  ...
  const handleSaveTasks = () => {
    ...
    axios.post('http://54.37.9.74:3001/todo', {"taches": tasksToSave, "commande_id":commandNumber, "taches_secondaire_id":taches_secondaires_id, "taches_principales_id":taches_principales_id}).catch((err) => {
      console.warn(err);
    });
