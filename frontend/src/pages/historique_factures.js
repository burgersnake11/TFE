...
const Historique_factures = () => {
  ...
  useEffect(() => {
    axios.get('https://studio-eventail.be:3001/factures', { withCredentials: true }).then((res) => {
      setFactures(res.data.rows);
    });
    axios.get('https://studio-eventail.be:3001/montant', { withCredentials: true }).then((res) => {
  ...
  function setPayer(id){
    axios.post("https://studio-eventail.be:3001/payer_facture", {"id":id}).catch(
  ...
  function archiver(){
      axios.post("https://studio-eventail.be:3001/archiver_facture", {"id":factureToDeleteId}).catch(
        err => console.warn(err)
    )
    window.location.reload()
  }
